...
class GameCalculator
{
    ...
    public $wildSymbols = ['0', '1'];
    public $scatterSymbol = '9';
    ...
    public function calculateSpin($postData)
    {
        ...
        for ($i = 0; $i <= 2000; $i++) {
            ...
            for ($k = 0; $k < $lines; $k++) {
                ...
                $lineSymbols = [];
                for ($r = 0; $r < 5; $r++) {
                    $lineSymbols[] = $reels['reel' . ($r + 1)][$line[$r] - 1];
                }
                ...
                if (in_array($firstSymbol, $this->wildSymbols)) {
                    $winSymbol = null;
                    foreach($lineSymbols as $s) {
                        if(!in_array($s, $this->wildSymbols)) {
                            $winSymbol = $s;
                            break;
                        }
                    }
                    if ($winSymbol === null) $winSymbol = $this->wildSymbols[0];
                }

                foreach ($lineSymbols as $s) {
                    if ($s == $winSymbol || in_array($s, $this->wildSymbols)) {
                        $winCount++;
                    } else {
                        break;
                    }
                ...
                if ($winCount > 0 && isset($this->Paytable['SYM_' . $winSymbol])) {
                    $pay = $this->Paytable['SYM_' . $winSymbol][$winCount] ?? 0;
                    if ($pay > 0) {
            ...
            if ($scattersCount >= 3 && $this->slotBonus) {
                $lineWins[] = '{"type":"Bonus","bonusName":"PickBonus","params":{"fields":"25","freeSpins":"8"},"amount":"' . $scattersWin . '","wonSymbols":[' . implode(',', $scattersPos) . ']}';
            }
